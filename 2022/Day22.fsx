...
let rec nextSqPt1 (map:Tile[,]) r c dr dc =
    ...
    | Wall -> r,c
    | Floor -> r',c'
    | Space -> let tr,tc = nextSqPt1 map r' c' dr dc
               if map[tr,tc] = Space then r,c
               else tr,tc

// A cursed function...
let rec nextSqPt2 (map:Tile[,]) r c dr dc =
    let h = map |> Array2D.length1
    let w = map |> Array2D.length2
    let r' = r+dr
    let c' = c+dc

    let fr, fc = if r' = -1 && c >= 100 then
                     // top edge of Panel 1
                     199, c'-100
                 elif c' = 150 then
...
let rec move (map:Tile[,]) (actions:(int*string)[]) r c  dr dc i f =
    if i < (Array.length actions) then
        ...
        let r', c' = seq { 1..steps }
                     |> Seq.fold(fun(r,c) _ -> f map r c dr dc) (r,c)
