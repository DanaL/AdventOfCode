...
let monkey0 (items: uint64 list) destress =
    ...
    items |> List.map(fun x -> destress (x * 13UL) % LCM)
...
let monkey1 (items: uint64 list) destress =
    ...
    items |> List.map(fun x -> destress (x + 3UL) % LCM)
...
let monkey2 (items: uint64 list) destress =
    ...
    items |> List.map(fun x -> destress (x + 6UL) % LCM)
...
let monkey3 (items: uint64 list) destress =
    ...
    items |> List.map(fun x -> destress (x + 2UL) % LCM) 
...
let monkey4 (items: uint64 list) destress =
    ...
    items |> List.map(fun x -> destress (x * x) % LCM)
...
let monkey5 (items: uint64 list) destress =
    ...
    items |> List.map(fun x -> destress (x + 4UL) % LCM)
...
let monkey6 (items: uint64 list) destress =
    ...
    items |> List.map(fun x -> destress (x * 7UL) % LCM)
